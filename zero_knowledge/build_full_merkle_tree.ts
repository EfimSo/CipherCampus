...
// 2) Build lookup maps:
const PROFESSOR_CODES: Record<string, number> =
  PROFESSORS.reduce((acc, name, idx) => {
    acc[name] = idx;
    return acc;
  }, {} as Record<string, number>);

const GRADE_CODES: Record<string, number> =
  GRADES.reduce((acc, grade, idx) => {
    acc[grade] = idx;
    return acc;
  }, {} as Record<string, number>);
...
class MerkleTree implements IMerkleTree {
  ...
  root() { return this.storage.get(`${this.levels}-0`) ?? this.zeros[this.levels]; }
  ...
  /** Full proof builder (not used in this script but keeps interface happy). */
  proof(index: number) {
...
const COURSE_FIXED: Record<string, number> = {
  "101": 0, "102": 1, "201": 2, "202": 3, "301": 4, "302": 5, "595": 6,
};
...
(async () => {
  ...
  for (const r of rows) {
    ...
    /* leaf hash = pedersen_hash([student_id, idx]) */
    const pk_x_big = BigInt(r.pk_x);
    const pk_y_big = BigInt(r.pk_y);

    const h1 = bb.pedersenHash(
      [toFr(pk_x_big), toFr(pk_y_big)],
      0,
    );

    const pCode = PROFESSOR_CODES[r.professor]; // e.g. 0–7
    const gCode = GRADE_CODES[r.grade];         // e.g. 0–10
  
    const fp = toFr(BigInt(pCode));
    const fg = toFr(BigInt(gCode));

    const h2 = bb.pedersenHash(
      [fp, fg],
      0,
    );

    const leaf = bb.pedersenHash(
      [h1, h2],
      0,
    );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      storage: Array.from(tree.storage.entries()).map(([k, v]) => [k, v.toString()]),
    }, null, 2),
  );

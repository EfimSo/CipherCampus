...
def add_bracket_annotation(label: str, key: str, value: str) -> str:
    pattern = r"^(.*?)\[(.*?)\]$"  
    match = re.match(pattern, label)
    if match:
        base_label = match.group(1)
        inside = match.group(2)
        new_inside = inside + f", {key}={value}"
        return f"{base_label}[{new_inside}]"
    else:
...
class Tree(object):
    ...
    def restore_unit(self: Type["Tree"]) -> None:
        ...
        def visit(node: Node) -> Node:
            ...
            labels: Sequence[str] = node.label.split('_')
    ...
    def binarize_right(self: Type["Tree"]) -> None:
        ...
        for node in nodes:
            if len(node.children) > 2:
                ...
                base_node_label = re.sub(r"\[.*?\]", "", node.label)

                prev: Node = children[0]
                for child in children[1:-1]:
                    child_base = re.sub(r"\[.*?\]", "", child.label)
                    new_label = f"{base_node_label}*"
                    new_label = add_bracket_annotation(new_label, "right", child_base)

                    prev = Node(new_label, [child, prev])
        ...
        for node in nodes:
            if len(node.children) > 2:
                ...
                for child in children[1:-1]:
                    child_base = re.sub(r"\[.*?\]", "", child.label)
                    new_label = f"{base_node_label}*"
                    new_label = add_bracket_annotation(new_label, "left", child_base)

                    prev = Node(new_label, [prev, child])
    ...
    def vertical_markovize(self) -> None:
        def _markovize_dfs(node: Node, parent_label: str) -> None:
            if node is None:
                return
            if parent_label is not None and node.children: 
                node.label = add_bracket_annotation(node.label, "mom", parent_label)

            current_base = re.sub(r"\[.*?\]", "", node.label)

            for child in node.children:
                _markovize_dfs(child, current_base)

        if self.root is not None:
            _markovize_dfs(self.root, None)


    def vertical_unmarkovize(self) -> None:
        def _unmarkovize_dfs(node: Node) -> None:
            if node is None:
                return
            node.label = re.sub(r"\[.*?\]", "", node.label)
            for child in node.children:
                _unmarkovize_dfs(child)

        if self.root:
            _unmarkovize_dfs(self.root)
